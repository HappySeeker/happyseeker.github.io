...
ARM架构提供16个32位的通用寄存器(R0-R15),R0-R14可用作普通的数据存储，R15为PC寄存器，修改R15可以改变程序的执行流程。R14也称LR(Link register)，通常用于保存当前函数的返回地址(上一级函数)，当其空闲时，也可以用作普通寄存器用。R13也称SP，即用于保存堆栈的栈顶指针，当其空闲时，也可以用作普通寄存器用。
